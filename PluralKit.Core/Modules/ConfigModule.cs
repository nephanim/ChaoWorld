...
namespace ChaoWorld.Core
{
    public class ConfigModule<T>: Module where T : new()
    {
        ...
        protected override void Load(ContainerBuilder builder)
        {
            ...
            builder.Register(c => c.Resolve<IConfiguration>().GetSection("ChaoWorld").Get<CoreConfig>() ?? new CoreConfig()).SingleInstance();

            // Register the submodule config (BotConfig, etc) if specified
            if (_submodule != null)
                builder.Register(c => c.Resolve<IConfiguration>().GetSection("ChaoWorld").GetSection(_submodule).Get<T>() ?? new T()).SingleInstance();
        }
    }
}