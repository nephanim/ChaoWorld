    ...
    public partial class BulkImporter
    {
        private async Task<ImportResultNew> ImportTupperbox(JObject importFile)
        {
            ...
            if (multipleTags || hasGroup)
            {
                ...
                if (hasGroup)
                    issueStr +=
                        "\n- ChaoWorld does not support member groups. Members will be imported without groups.";
                if (multipleTags)
                    issueStr +=
                        "\n- ChaoWorld does not support per-member system tags. Since you had multiple members with distinct tags, those tags will be applied to the members' *display names*/nicknames instead.";
        ...
        private async Task<(string lastSetTag, bool multipleTags, bool hasGroup)> ImportTupper(JObject tupper, string lastSetTag)
        {
            ...
            _logger.Debug("Importing member with identifier {FileId} to system {Garden} (is creating new member? {IsCreatingNewMember})",
