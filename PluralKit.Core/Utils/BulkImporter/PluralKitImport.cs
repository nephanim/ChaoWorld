...
namespace ChaoWorld.Core
{
    public partial class BulkImporter
    {
        ...
        private async Task ImportMember(JObject member)
        {
            ...
            _logger.Debug(
                "Importing member with identifier {FileId} to system {Garden} (is creating new member? {IsCreatingNewMember})",
        ...
        private async Task ImportGroup(JObject group)
        {
            ...
            _logger.Debug(
                "Importing group with identifier {FileId} to system {Garden} (is creating new group? {IsCreatingNewGroup})",
            ...
            var currentGroupMembers = (await _conn.QueryAsync<ChaoId>(
        ...
        {
            var existingSwitches = (await _conn.QueryAsync<PKSwitch>("select * from switches where system = @Garden", new { System = _system.Id })).ToList();
            ...
            var justAddedSwitches = await _conn.QueryAsync<PKSwitch>(
                "select * from switches where system = @Garden and id > @LastSwitchId",
