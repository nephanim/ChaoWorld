...
namespace ChaoWorld.Bot
{
    ...
    public static class Errors
    {
        ...
        public static PKError NoSystemError => new PKError("You do not have a system registered with ChaoWorld. To create one, type `pk;system new`.");
        public static PKError ExistingSystemError => new PKError("You already have a system registered with ChaoWorld. To view it, type `pk;system`. If you'd like to delete your system and start anew, type `pk;system delete`, or if you'd like to unlink this account from it, type `pk;unlink`.");
        ...
        public static PKError MemberLimitReachedError(int limit) => new PKError($"Garden has reached the maximum number of members ({limit}). Please delete unused members first in order to create new ones.");
        ...
        public static PKError AccountInOtherSystem(Garden system) => new PKError($"The mentioned account is already linked to another system (see `pk;system {system.Hid}`).");
        ...
        public static PKError SameSwitch(ICollection<Chao> members, LookupContext ctx)
        {
        ...
        public static PKError InvalidImportFile => new PKError("Imported data file invalid. Make sure this is a .json file directly exported from ChaoWorld or Tupperbox.");
        public static PKError ImportCancelled => new PKError("Import cancelled.");
        public static PKError MessageNotFound(ulong id) => new PKError($"Message with ID '{id}' not found. Are you sure it's a message proxied by ChaoWorld?");
        ...
        public static PKError ProxyTagAlreadyExists(ProxyTag tagToAdd, Chao member) => new PKError($"That member already has the proxy tag {tagToAdd.ProxyString.AsCode()}. The member currently has these tags: {member.ProxyTagsString()}");
        public static PKError ProxyTagDoesNotExist(ProxyTag tagToRemove, Chao member) => new PKError($"That member does not have the proxy tag {tagToRemove.ProxyString.AsCode()}. The member currently has these tags: {member.ProxyTagsString()}");
        public static PKError LegacyAlreadyHasProxyTag(ProxyTag requested, Chao member) => new PKError($"This member already has more than one proxy tag set: {member.ProxyTagsString()}\nConsider using the {$"pk;member {member.Reference()} proxy add {requested.ProxyString}".AsCode()} command instead.");
        public static PKError EmptyProxyTags(Chao member) => new PKError($"The example proxy `text` is equivalent to having no proxy tags at all, since there are no symbols or brackets on either end. If you'd like to clear your proxy tags, use `pk;member {member.Reference()} proxy clear`.");

        public static PKError GenericCancelled() => new PKError("Operation cancelled.");

        public static PKError AttachmentTooLarge => new PKError("ChaoWorld cannot proxy attachments over 8 megabytes (as webhooks aren't considered as having Discord Nitro) :(");
