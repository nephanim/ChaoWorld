...
namespace ChaoWorld.Bot
{
    public static class ContextPrivacyExt
    {
        public static PrivacyLevel PopPrivacyLevel(this Context ctx)
        {
            ...
            if (!ctx.HasNext())
                throw new CWSyntaxError("You must pass a privacy level (`public` or `private`)");

            throw new CWSyntaxError($"Invalid privacy level {ctx.PopArgument().AsCode()} (must be `public` or `private`).");
        }
        ...
        public static SystemPrivacySubject PopSystemPrivacySubject(this Context ctx)
        {
            if (!SystemPrivacyUtils.TryParseSystemPrivacy(ctx.PeekArgument(), out var subject))
                throw new CWSyntaxError($"Invalid privacy subject {ctx.PopArgument().AsCode()} (must be `description`, `members`, `front`, `fronthistory`, `groups`, or `all`).");
        ...
        public static MemberPrivacySubject PopMemberPrivacySubject(this Context ctx)
        {
            if (!MemberPrivacyUtils.TryParseMemberPrivacy(ctx.PeekArgument(), out var subject))
                throw new CWSyntaxError($"Invalid privacy subject {ctx.PopArgument().AsCode()} (must be `name`, `description`, `avatar`, `birthday`, `pronouns`, `metadata`, `visibility`, or `all`).");
        ...
        public static GroupPrivacySubject PopGroupPrivacySubject(this Context ctx)
        {
            if (!GroupPrivacyUtils.TryParseGroupPrivacy(ctx.PeekArgument(), out var subject))
                throw new CWSyntaxError($"Invalid privacy subject {ctx.PopArgument().AsCode()} (must be `description`, `icon`, `visibility`, or `all`).");
