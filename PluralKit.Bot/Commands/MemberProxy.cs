...
namespace ChaoWorld.Bot
{
    public class MemberProxy
    {
        ...
        public async Task Proxy(Context ctx, Chao target)
        {
            ...
            {
                var query = "select * from (select *, (unnest(proxy_tags)).prefix as prefix, (unnest(proxy_tags)).suffix as suffix from members where system = @Garden) as _ where prefix is not distinct from @Prefix and suffix is not distinct from @Suffix and id != @Existing";
                var conflicts = (await _db.Execute(conn => conn.QueryAsync<Chao>(query,
                    new { Prefix = newTag.Prefix, Suffix = newTag.Suffix, Existing = target.Id, system = target.Garden }))).ToList();
