    ...
    public class Item
    {
        ...
        private async Task<bool> TryHandleUseFruit(Context ctx, Core.Item item, Core.Chao chao, int quantity)
        {
            ...
            if (chao != null)
            {
                ...
                var reduceHunger = new Random().Next(5, 11) * quantity; // If eating the fruit worked, it should reduce hunger a little
                chao.Hunger = Math.Max(0, chao.Hunger - reduceHunger);
