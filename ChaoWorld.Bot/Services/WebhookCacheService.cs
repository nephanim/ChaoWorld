    ...
    public class WebhookCacheService
    {
        ...
        private async Task<Webhook?> GetOrCreateWebhook(ulong channelId)
        {
            ...
            if (webhooks.Length >= 10)
                throw new CWError("This channel has the maximum amount of possible webhooks (10) already created. A server admin must delete one or more webhooks so Chao World can create one.");
