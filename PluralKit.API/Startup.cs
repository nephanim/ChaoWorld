    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddAuthorization(options =>
            {
                ...
                options.AddPolicy("ViewMembers", p => p.AddRequirements(new PrivacyRequirement<Garden>(s => s.MemberListPrivacy)));
                options.AddPolicy("ViewFront", p => p.AddRequirements(new PrivacyRequirement<Garden>(s => s.FrontPrivacy)));
                options.AddPolicy("ViewFrontHistory", p => p.AddRequirements(new PrivacyRequirement<Garden>(s => s.FrontHistoryPrivacy)));
            });
            ...
            {
                c.SwaggerDoc("v1.0", new OpenApiInfo { Title = "ChaoWorld", Version = "1.0" });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            if (env.IsDevelopment())
            {
                ...
                app.UseSwaggerUI(c =>
                {
                    c.SwaggerEndpoint("/swagger/v1.0/swagger.json", "ChaoWorld (v1)");
                });
            }
            else
            {
